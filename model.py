    ...
    def forward(self, uids, iids, pos, neg, test=False, test_items=None, num_negative=100):
        if test==True:  # testing phase
            ...
            for uid in uids:
                ...
                all_items = set(range(self.E_i.shape[0]))  
                interacted_items = set(self.train_csr[uid_cpu].indices)  
                non_interacted_items = all_items - interacted_items

                positive_item = test_items[uid_cpu]
                possible_negatives = list(non_interacted_items - set(positive_item))
                
                if not possible_negatives:
                    continue 
        
                if len(possible_negatives) >= num_negative:
                    sampled_negatives = np.random.choice(possible_negatives, size=num_negative, replace=False)
                else:
                ...
                sorted_preds = preds.argsort(descending=True)
                predictions.append(items_to_predict[sorted_preds.cpu().numpy()])  # Chuyển sorted_preds về CPU trước khi sử dụng làm chỉ số
            
       
            predictions = np.array(predictions)
            predictions = torch.tensor(predictions, device=uids.device)  # Đảm bảo `predictions` nằm trên cùng thiết bị với `uids`
